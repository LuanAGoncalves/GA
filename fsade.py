...
class DE(object):
    ...
    def __init__(self, min_real, x='rand', y=1, z='bin', *, F=.5, CR=.1,probi=random.uniform(.1,1)):
        self.x = x
        self.y = y
        self.z = z
        self.F = F
    ...
    def _set_probi(self, probi):
        if not 0.1 <= CR <= 1:
            raise ValueError('probi should belong to [0.1, 1].')
