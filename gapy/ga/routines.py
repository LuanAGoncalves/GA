...
import random as rd
import numpy as np
from ..faux import faux
import chromossome as ch
...
def newGeneration (nvar, lvar, populationSize, ls, li):
	...
	while i < populationSize: 
		generation += [ch.Chromossome(nvar, lvar, ls, li, map(lambda j: str(x[j]*rd.randrange(0,2)), range(len(x))))]
		i += 1
	...
	for i in range(len(generation)):
		if i == 0:
			...
			generation[i].sl = [generation[i-1].sl[1], generation[i-1].sl[1] + generation[i].fitness/total]
...
def crossingOnePoint (nvar, lvar, tc, generation, ls, li):
	...
	while i < (len(generation)/2):
		...
		p1 = faux.list2str(p[0])
		p2 = faux.list2str(p[1])
		...
		if rd.random() <= tc:
			...
			cGeneration += [ch.Chromossome(nvar, lvar, ls, li, list(b1)), ch.Chromossome(nvar, lvar, ls, li, list(b2))]

		else:
			cGeneration += [ch.Chromossome(nvar, lvar, ls, li, list(p1)), ch.Chromossome(nvar, lvar, ls, li, list(p2))]
		i += 1
	...
	for i in range(len(cGeneration)):
		if i == 0:
			...
			cGeneration[i].sl = [cGeneration[i-1].sl[1], cGeneration[i-1].sl[1] + cGeneration[i].fitness/total]
